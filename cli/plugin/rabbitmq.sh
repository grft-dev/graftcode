...
EOF
  ...
  say "Graftcode RabbitMQ Plugin installer"
  say ""
  say "This script downloads and installs the RabbitMQ plugin"
...
install_plugin_files() {
 ...
 - $(basename "$file")"
  ...
  for pattern in libRabbitmqPlugin.so libRabbitmqPlugin.dylib RabbitmqPlugin.dll; do
 ...
 - $(basename "$found")"
  ...
  if [ -z "$installed" ]; then
    say "Could not find RabbitMQ plugin binaries inside archive."
    say "Extracted files:"
...
install_rabbitmq() {
  ...
  if [ -z "$asset_url" ]; then
    say "Could not find RabbitMQ build: $asset_name"
    say ""
    say "Available rabbitmq assets:"
    grep '"browser_download_url"' "$release_json" |
      sed 's/.*"browser_download_url": "\(.*\)".*/\1/' |
      grep -Ei '/rabbitmq-' |
  ...
  say ""
  say "Installed Graftcode RabbitMQ plugin in:"
  say "$INSTALL_DIR"
}
